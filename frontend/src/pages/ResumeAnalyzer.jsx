...
import { Upload, FileText, Trash2, Search, BrainCircuit, AlertCircle, CheckCircle, ArrowRight, Loader } from 'lucide-react';
...
export default function ResumeAnalyzer() {
    ...
    const handleAnalyze = async (resume) => {
        ...
        try {
            ...
            try {
                const existing = await resumeService.getAnalysis(resume.id);
                if (existing.success) {
            ...
            const data = await resumeService.analyzeResume(resume.id, "Software Engineer"); // Default target for now
            if (data.success) {
